...
CREATE VIEW step_3 AS (
    ...
    LEFT JOIN vk_date_spent AS vds
        ON
            ...
            AND vds.utm_source = pv.source
            AND vds.utm_medium = pv.medium
            AND vds.utm_campaign = pv.campaign
    LEFT JOIN ya_date_spent AS yds
        ON
            yds.day_ya = DATE(pv.visit_date)
            AND yds.utm_source = pv.source
            AND yds.utm_medium = pv.medium
            AND yds.utm_campaign = pv.campaign
    GROUP BY 1, 2, 3, 4, 6
...
WITH tab AS (
    SELECT
        visit_date,
        COALESCE(SUM(total_cost) FILTER (WHERE utm_source = 'vk'), 0) AS daily_cost_vk,
        COALESCE(SUM(total_cost) FILTER (WHERE utm_source = 'yandex'), 0) AS daily_cost_ya
    FROM step_3
    GROUP BY 1
    ORDER BY 1
)
